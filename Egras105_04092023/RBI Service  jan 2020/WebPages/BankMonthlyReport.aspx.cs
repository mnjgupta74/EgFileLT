...
public partial class WebPages_BankMonthlyReport : System.Web.UI.Page
{
    ...
    protected void btnUpload_Click(object sender, EventArgs e)
    {
        ...
        string Challandate = (Cdate[1].ToString().Substring(2, 4) + "/" + Cdate[1].ToString().Substring(0, 2) + "/" + "01");
        if (FileUpload1.HasFile && FileUpload1.PostedFile != null && FileUpload1.PostedFile.FileName != "")
        {
            if (FileUpload1.PostedFile.ContentLength > 0 && Path.GetExtension(FileUpload1.PostedFile.FileName).ToLower().Equals(".xml"))
            {
