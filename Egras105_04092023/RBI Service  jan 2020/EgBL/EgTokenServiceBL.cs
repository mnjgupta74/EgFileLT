    ...
    public class EgTokenServiceBL
    {
        ...
        public string VeryfyTokenData()
        {
            try
            {
                ...
                string PlainString = objDecrypt.DecryptSBIWithKey256(encdata, System.Web.Configuration.WebConfigurationManager.AppSettings["Securekey"] + ObjSNAMerchantCode.GetBankName(merchantcode) + ".key", null);
        ...
        public string verifyUser()
        {
            ...
            try
            {
                ...
                return str.Length > 1 ? JsonConvert.SerializeObject(new TokenMessage { msg = "success", token = str }) : JsonConvert.SerializeObject(new TokenMessage { msg = "Token Not Generated !" });
            }
            catch (Exception ex)
            {
                EgErrorHandller obj = new EgErrorHandller();
