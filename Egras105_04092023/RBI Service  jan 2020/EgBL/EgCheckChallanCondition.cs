...
namespace EgBL
{
    ...
    {        
        ...
        public bool CheckSubmitCondition()
        {
            ...
            if (pdAccountNo != "0" && Convert.ToInt32(MaxBudgetHead.Substring(0, 4)) > 7999 && 
                (payMode == "N"))
            {
                if (isPD)
                {
                ...
                else if ((proc_id == -1 || proc_id == -2) && (serviceBudgetHead.Contains("8443001030000") || 
                    serviceBudgetHead.Contains("8443001080000") || serviceBudgetHead.Contains("8443001090000")) && !isPD)
                {
                    return false;
                }
            ...
            else if (payMode == "M" && pdAccountNo != "0" )
                ///*&& treasuryCodeBank != treasuryCodePd && treasuryCodeBank.Substring(7, 2) != "-1"*/)
            {
                ...
                PM[0] = new SqlParameter("@pdAccountNo", SqlDbType.VarChar,10) { Value = pdAccountNo };
                Int16 i= Convert.ToInt16(gf.ExecuteScaler(PM, "EgCheckPdTreasuryMapping"));
                if (i == 0)
                {
                    retrunValue = true;
                    msg = "Pd / Division does not match with selected treasury.";
            ...
            else if ((proc_id == -1 || proc_id == -2) && (serviceBudgetHead.Contains("8443001080000") || 
                serviceBudgetHead.Contains("8443001090000")) && (payMode == "N" || payMode == "M" ))
            {
            ...
            else if((BudgetHead== "8443001080000" || BudgetHead == "8443001090000" )&& 
                (payMode == "N"  || payMode == "M") && pdAccountNo=="0")
            {
                retrunValue = true;
                msg = "please select Pd / Division Code.";
            }

            else if (BudgetHead != "8443001030000" && (BudgetHead.Substring(0, 4) == "8782" || BudgetHead.Substring(0, 4) == "8443" || 
                BudgetHead == "8448001200708" || BudgetHead == "8448001200900") && (payMode == "N"))
            {
                retrunValue = true;
                msg = "E-Banking is not allowed in this Case. Please select 0215 for depositing amount.";
            }
