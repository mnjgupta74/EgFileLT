...
/// <summary>
/// Summary description for IDBI
/// </summary>
public class IDBI : Banks
{
    public IDBI()
    {
        KeyName = "IDBI";
    ...
    public override string CallVerifyService(string cipherText)
    {
        try
        {
            using (IDBIWebServ.Service idbiService = new IDBIWebServ.Service())
            {
                ...
                return idbiService.UpdateRajEgrasInquiry(cipherText);
            }
        }
        catch (Exception ex)
        {
            InsertErrorLog(ex.Message, "6910213", ConfigurationManager.AppSettings["IDBIWebServ.inquiryservice"].Split('/').GetValue(2).ToString().Replace("www.", ""), 1);
            return null;
        }
    }
