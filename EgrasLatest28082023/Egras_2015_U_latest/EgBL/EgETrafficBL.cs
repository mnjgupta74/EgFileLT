...
namespace EgBL
{
    public class EgETrafficBL
    {
        ...
        {
            if (ChallanValues[22][1].ToString() == null || ChallanValues[22][1].ToString() == ""
                || Mcode.ToString() == "" || Mcode.ToString() == null)
            {
                throw new Exception("Merchant Code is Not Valid");
            }
        ...
        //Registration No Identity Sql Injection Check
        ...
        public void CheckIdentity()
        {
            ...
            //!ra.IsMatch(txtTIN.Text.Trim()
            if (!r.IsMatch(ChallanValues[23][1].ToString()))
            {            
                throw new Exception("Registration No not Valid");
            }
        }
        //ChallanValues[24][1].ToString()

        // Amount Checking Head Amount And Total Amount Should be  same
        public void CheckAmount()
        {

            if (amount(ChallanValues[21][1].ToString()) < 0 && amount(ChallanValues[2][1].ToString()) < 0)
            {
        ...
        public int insertResponseLog()
        {
            int Rv;
            //int Mcode, string AUIN,string EncData,string Response
            gf = new GenralFunction();
            SqlParameter[] PM = new SqlParameter[3];
            PM[0] = new SqlParameter("@Mcode", SqlDbType.Int) { Value = Mcode };
            PM[1] = new SqlParameter("@AUIN", SqlDbType.VarChar, 50) { Value = AUIN };
            PM[2] = new SqlParameter("@Response", SqlDbType.VarChar, 100) { Value = Response };
            Rv = gf.UpdateData(PM, "EginsertAuditMerchantLog");
            return Rv;
        }
        ...
        public int InsertEge_TrafficDynamicChallan()
        {
            ...
            try
            {
                ...
                PM[22] = new SqlParameter("@IpAddress", SqlDbType.NVarChar, 50) { Value = HttpContext.Current.Request.ServerVariables[ConfigurationManager.AppSettings["IPAddressHTTP"]].ToString() };//System.Web.HttpContext.Current.Request.ServerVariables["REMOTE_ADDR"].ToString() };
