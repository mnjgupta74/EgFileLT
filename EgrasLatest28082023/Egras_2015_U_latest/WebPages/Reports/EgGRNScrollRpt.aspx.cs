...
public partial class WebPages_Reports_EgGRNScrollRpt : System.Web.UI.Page
{
    ...
    protected void btnMsExcel_Click(object sender, EventArgs e)
    {
            ...
            try
            {
                ...
                Response.AddHeader("content-disposition", "attachment;filename=BankScroll.xls");
                Response.Charset = "";
    ...
    protected void grdScroll_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                SumEAmount = Convert.ToDouble(dt.Compute("Sum(ETreasuryAmount)", ""));
                SumBAmount = Convert.ToDouble(dt.Compute("Sum(ScrollAmount)", ""));
                TotalENoOfTransactions = dt.Select("ETreasuryAmount > 0").Length;
                TotalBNoOfTransactions = dt.Select("ScrollAmount > 0").Length;
                grdScroll.DataSource = dt;
            ...
            TAmountTotal += Convert.ToDecimal(DataBinder.Eval(e.Row.DataItem, "ScrollAmount"));
            BAmountTotal += Convert.ToDecimal(DataBinder.Eval(e.Row.DataItem, "ETreasuryAmount"));
        }
