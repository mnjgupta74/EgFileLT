...
public partial class WebPages_Reports_SignedPDF : System.Web.UI.Page
{
    ...
    protected void btnShow_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (rblFileType.SelectedValue != "3")
            {
    ...
    private void BindGrid()
    {
        try
        {
            ...
            if (rblFileType.SelectedValue == "1")
            {
                ...
                pnldgekSign.Visible = false;
                grddailyFile.Visible = false;
                pnldailyFile.Visible = false;
            ...
            else if (rblFileType.SelectedValue == "2")
            {
                dgekSignDtls_BankUpload.Visible = false;
                pnldgekSign_BankUpload.Visible = false;
                dgekSignDtls.Visible = false;
                ...
                if (rblFileType.SelectedValue == "3")
            {
                ...
                DateTime fromdate = DateTime.ParseExact(txtfromdate.Text, "dd/MM/yyyy", null).Date;
                DateTime todate = DateTime.ParseExact(txtfromdate.Text, "dd/MM/yyyy", null).Date; //DateTime.ParseExact(txttodate.Text, "dd/MM/yyyy", null).Date;
                string[] Rdate = null;
                Rdate = txtfromdate.Text.Split("/".ToCharArray());
                string[] Tdate = null;
                Tdate = txtfromdate.Text.Split("/".ToCharArray());

                objAGSReport_BL.Filedate = Convert.ToDateTime(Rdate[2] + "-" + Rdate[1] + "-" + Rdate[0]);
                objAGSReport_BL.Todate = Convert.ToDateTime(Tdate[2] + "-" + Tdate[1] + "-" + Tdate[0]); ;
                //objAGSReport_BL.Phase = Convert.ToInt16(ddlPhase.SelectedValue);

                ds = objAGSReport_BL.GetDailyFilesGrid();
                grddailyFile.DataSource = null;
                grddailyFile.DataBind();
                dgekSignDtls_BankUpload.Visible = false;
                pnldgekSign_BankUpload.Visible = false;
                grddailyFile.Visible = false;
                pnldailyFile.Visible = false;
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    string FTPDate = ds.Tables[0].Rows[0]["FTPDate"].ToString();
                    string TypeOfData = ds.Tables[0].Rows[0]["TypeOfData"].ToString();
                    dgekSignDtls_BankUpload.Visible = false;
                    pnldgekSign_BankUpload.Visible = false;
                    pnldgekSign.Visible = false;
                    dgekSignDtls.Visible = false;
                    grddailyFile.Visible = true;
                    pnldailyFile.Visible = true;
                    grddailyFile.DataSource = ds;
                    grddailyFile.DataBind();

                    ds.Dispose();
                    objAGSReport_BL = null;
                    if(FTPDate == "" && TypeOfData == "1")
                    {
                        btnFTP.Visible = true;
                    }
                    else
    ...
    protected void btnFTP_Click(object sender, EventArgs e)
    {
        ...
        {
            EgAGSignedReportBL objAGSReport_BL = new EgAGSignedReportBL();
            string fileDate = txtfromdate.Text;
            //DateTime fildate  = DateTime.Parse(txtfromdate.Text);
            //string[] FileDate = null;
            //FileDate = txtfromdate.Text.Split("/".ToCharArray());

            //objAGSReport_BL.Filedate = Convert.ToDateTime(FileDate[2] + "-" + FileDate[1] + "-" + FileDate[0]);

            int i = objAGSReport_BL.FTPTransfer(fileDate);
            if (i == 1)
            {
                BindGrid();
                ScriptManager.RegisterClientScriptBlock(this.Page, this.GetType(), "Msg", "alert('Success Response Received From ImServer');", true);
            }
            else
            {
                ScriptManager.RegisterClientScriptBlock(this.Page, this.GetType(), "Msg", "alert('FAIL Response Received From  IMServer');", true);
            }
            BindGrid();
        }
        catch (Exception ex)
        ...
        if (e.CommandName == "eSign")
        {
            try
            {
                ...
                else
                {
                    ...
                    objAGSReport_BL.IPAddress = System.Web.HttpContext.Current.Request.ServerVariables[ConfigurationManager.AppSettings["IPAddressHTTP"]].ToString(); ;//Request.ServerVariables["REMOTE_ADDR"].ToString();
    ...
    protected void rblFileType_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (rblFileType.SelectedValue == "2")
        {
            txttodate.Enabled = true;
            ddlMonth.SelectedIndex = 0;
            ddlYear.SelectedIndex = 0;
        ...
        else if (rblFileType.SelectedValue == "3")
        {
            //tdDate
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (rblFileType.SelectedValue == "1")
                    {
                        ...
                        cert = new X509Certificate2(System.Web.Configuration.WebConfigurationManager.AppSettings["SecureCertificate"] + ConfigurationManager.AppSettings["Certificate"].ToString(), ConfigurationManager.AppSettings["CertificatePassword"].ToString());
                        PDFSign objpdfsign = new PDFSign();
    ...
    protected void grddailyFile_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        if (e.CommandName == "eSign_Daily")
        {
            try
            {
                EgAGSignedReportBL objAGSReport_BL = new EgAGSignedReportBL();
                ...
                Button lnk = (e.CommandSource) as Button;
                GridViewRow clickedRow = lnk.NamingContainer as GridViewRow;
                Label lblReportName = (clickedRow.FindControl("lblFNameDaily") as Label);
                hidID.Value = ID.ToString();
                objAGSReport_BL.ID = Convert.ToInt64(hidID.Value);


                ds = objAGSReport_BL.DownloadFileDaily();
                if (ds.Tables.Count > 0)
                {
    ...
    protected void grddailyFile_SelectedIndexChanged(object sender, EventArgs e)
    {
        for (int i = 0; i < grddailyFile.Rows.Count; i++)
        {
            Button btneSignDaily = grddailyFile.Rows[i].FindControl("btneSignDaily") as Button;
            btneSignDaily.Enabled = false;
            btneSignDaily.BackColor = System.Drawing.Color.Green;
            string eSignDate = grddailyFile.SelectedRow.Cells[7].Text.ToString().Trim().Replace("&nbsp;", "");

            Button btneSign = grddailyFile.Rows[grddailyFile.SelectedIndex].FindControl("btneSignDaily") as Button;
            btneSign.Enabled = false;
            btneSign.BackColor = System.Drawing.Color.Green;

            //Int64 ID = Convert.ToInt64(grddailyFile.SelectedItem.Cells[1].Text.ToString().Trim());
            hidID.Value = grddailyFile.SelectedRow.Cells[1].Text.ToString().Trim();
            Session["hidID"] = hidID.Value;
    ...
    protected void grddailyFile_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            Button btneSignDaily = (e.Row.FindControl("btneSignDaily") as Button);
            HtmlTableRow tr = (HtmlTableRow)e.Row.FindControl("trIDDaily");
            DataRowView drv = e.Row.DataItem as DataRowView;
            string Flag = Convert.ToString(drv["eSignDate"]);
            if (Flag == "")
            {
                btneSignDaily.BackColor = Color.FromArgb(0, 153, 0);
            }
        }
    }
}