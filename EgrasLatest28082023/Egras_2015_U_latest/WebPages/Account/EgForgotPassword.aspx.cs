    ...
    public bool send(string uid, string password, string message, string no, string SENDERID , string dlt_entity, string dlt_tempate_id)
    {
       ...
       // string responseString = SMSservice.GetSMSDetails(cipherText);
        HttpWebRequest myReq =
            (HttpWebRequest)WebRequest.Create("https://smsgw.sms.gov.in/failsafe/HttpLink?username=" + uid + "&pin=" + password + "&message=" + "Egras Password Reset Code:loginID:" + ViewState["LoginId"].ToString() + " and OTP:" + message + "&mnumber=" + no + "&signature=" + SENDERID + "&dlt_entity_id=" + dlt_entity+ "&dlt_template_id=" + dlt_tempate_id);
