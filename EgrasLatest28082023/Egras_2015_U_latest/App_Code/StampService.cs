...
{   
    ...
    public string GRN_Verify_Data(string encGRN, int MerchantCode)
    {
        try
        {
                ...
                if (lstValues[15].ToString() == "P")
                {
                    ...
                    if (returnData != "" && returnData != null)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                CheckSum = objEncryptDecryptionBL.GetMD5Hash("AUIN=0|CIN=0|BankReferenceNo=0|BANK_CODE=0|BankDate=0|GRN=" + VID.ToString() + "|Amount=" + Amount.ToString("F") + "|Status=P");
                            ...
                            if (lstReturnData[13].ToString() == "S" || lstReturnData[13].ToString() == "F")
                            {
                                Result = Convert.ToString(SqlHelper.ExecuteScalar(SqlHelper.conString, "GetGrnMerchantDetails_ws", PM));
                            }
                        }
            ...
            {
                CheckSum = objEncryptDecryptionBL.GetMD5Hash("AUIN=0|CIN=0|BankReferenceNo=0|BANK_CODE=0|BankDate=0|GRN=" + VID.ToString() + "|Amount=0.00|Status=F");
    ...
    public string AUIN_Verify_Data(string encAUIN, int MerchantCode)
    {
        try
        {
            ...
            cipherText = objEncry.Encrypt(Result, System.Web.Configuration.WebConfigurationManager.AppSettings["Securekey"] + MerchantCode + ".key");
            //stampLog.InsertStampLog(MerchantCode, cipherText, VID, 0, "OU");
        }
        catch (Exception ex)

        {
            cipherText = "Request Unable To Process !";
            EgErrorHandller obj = new EgErrorHandller();
            obj.InsertError(MerchantCode + "-" + encAUIN + "-" + ex.Message.ToString());
        }
        return cipherText;
    }
    /// <summary>
