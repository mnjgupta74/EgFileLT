...
public class EgTrgService : System.Web.Services.WebService
{
    ...
    public string GetSMSDetails(string Vcode)
    {
        ...
        HttpWebRequest myReq =
            (HttpWebRequest)WebRequest.Create("https://smsgw.sms.gov.in/failsafe/HttpLink?username=" + "egrasraj.otp" + "&pin=" + "T3%23uB7%24xD0" + "&message=" + "Egras Password Reset Code:loginID:"+ "***" +" and OTP:" + Verfcode.Trim() + "&mnumber=" + Mobile.ToString().Trim() + "&signature=" + "EGRASJ"+ "&dlt_entity_id=" + dlt_entity + "&dlt_template_id=" + dlt_tempate_id);
    ...
    /// <summary>
    /// Service Consume By SSp Integration
    ///  5025 Merchant For Paymanager
    ///  5018 For SSP
    /// </summary>
    ...
    [WebMethod]
    public Byte[] GetMinusExpenditureChallan(string encData, int MerchantCode)
    {
        SbiEncryptionDecryption objEncry = new SbiEncryptionDecryption();
        string PlainText = "";
        ...
        try
        {
            ...
            if (MerchantCode == 5018)
            {
                PlainText = objEncry.DecryptAES256(encData, System.Web.Configuration.WebConfigurationManager.AppSettings["Securekey"] + MerchantCode + ".key", null);
            }
            else
            {
                PlainText = objEncry.Decrypt(encData, System.Web.Configuration.WebConfigurationManager.AppSettings["Securekey"] + MerchantCode + ".key");
            }     
            List<string> lstPlainText = new List<string>();
            string[] arrMsgs = PlainText.Split('=');
