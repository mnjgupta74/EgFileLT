...
public class OBC : Banks
{
    ...
    public override string CallVerifyService(string cipherText)
    {
        try
        {
            using (OBCWebServ.RajasthanVerifyWebServiceImplService OBCService = new OBCWebServ.RajasthanVerifyWebServiceImplService())
            {
                ...
                return OBCService.rajverificationrequest(cipherText);
            }
        }
        catch (Exception ex)
        {
            InsertErrorLog(ex.Message, "0361193", ConfigurationManager.AppSettings["OBCWebServ.RajasthanVerifyWebServiceImplService"].Split('/').GetValue(2).ToString().Replace("www.", ""), 1);
            return null;
        }
    }
