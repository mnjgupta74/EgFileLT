            ...
            if (PaymentType == "M")
            {
                switch (BankCode.Substring(0, 7))
                {
                    case "0304017":
                        ...
                        InsertAudit(plainText, BankCode, ConfigurationManager.AppSettings["ServerName"].ToString() + " => " + cipherText, flag);
                    ...
                    case "0006326":
                        plainText = plainText + "|checkSum=" + checkSum;
                        cipherText = objEncry.Encrypt(plainText, System.Web.Configuration.WebConfigurationManager.AppSettings["Securekey"]+ "RAJASTHAN_EGRASS_Manual.key");
                        InsertAudit(plainText, BankCode, ConfigurationManager.AppSettings["ServerName"].ToString() + " => " + cipherText, flag);
                       ...
                       // SBIManualServ.RAJASTHANWS obj = new SBIManualServ.RAJASTHANWS();
                        ...
                        UploadData(BankCode, returnData, IPAddress,GRN,PaymentType);
                        plainText = objEncry.Decrypt(returnData, System.Web.Configuration.WebConfigurationManager.AppSettings["Securekey"]+ "RAJASTHAN_EGRASS_Manual.key");
                        message = UpdateStatus(plainText, GRN.ToString(), amt.ToString());
                        break;
                    ...
                    case "0200113":
                        plainText = plainText + "|checkSum=" + checkSum;
                        cipherText = objEncry.EncryptAES256(plainText, System.Web.Configuration.WebConfigurationManager.AppSettings["Securekey"] + "BOB1.key");
                        InsertAudit(plainText, BankCode, ConfigurationManager.AppSettings["ServerName"].ToString() + " => " + cipherText, flag);
                        BankUrl = System.Web.Configuration.WebConfigurationManager.AppSettings["BOBM"];
                        //CBIManualServVerify.graswebservice CBIManualService = new CBIManualServVerify.graswebservice();
                       // returnData = CBIManualService.verifyGRASData(cipherText);
                        ...
                        try
                        {
                            using (WebClient webClient1 = new WebClient())
                            {
                                ...
                                returnData= result.eChallanPaymentInqResp.value;
                                IPAddress =  Dns.GetHostByName(System.Web.Configuration.WebConfigurationManager.AppSettings["BOBM"]).AddressList[0].ToString();
                                UploadData(BankCode, returnData, IPAddress, GRN, PaymentType);
                                plainText = objEncry.DecryptAES256(returnData, System.Web.Configuration.WebConfigurationManager.AppSettings["Securekey"] + "BOB1.key",null);
                                message = UpdateStatus(plainText, GRN.ToString(), amt.ToString());
            ...
            else
            {
                switch (BankCode.Substring(0,7))
                {
                    case "0200113":
                        ...
                        BOBWebServ.WebService bobService  = new BOBWebServ.WebService();
                        ServicePointManager.CertificatePolicy = new TrustAllCertificatePolicy();
                    ...
                    case "0006326":
                        ...
                        plainText = plainText + "|checkSum=" + checkSum;
                        cipherText = objEncry.EncryptAES256(plainText, System.Web.Configuration.WebConfigurationManager.AppSettings["Securekey"]+"RAJASTHAN_EGRASS.key");
                        InsertAudit(plainText, BankCode, ConfigurationManager.AppSettings["ServerName"].ToString() + " => " + cipherText, flag);
                        BankUrl = ConfigurationManager.AppSettings["SBIWebServ.DoubleVerifyMerchantService"].Split('/').GetValue(2).ToString().Replace("merchant.", "");
                        ...
                        IPAddress = Dns.GetHostByName(ConfigurationManager.AppSettings["SBIWebServ.DoubleVerifyMerchantService"].Split('/').GetValue(2).ToString().Replace("merchant.", "")).AddressList[0].ToString();
                        UploadData(BankCode, returnData, IPAddress, GRN, PaymentType);
                        plainText = objEncry.DecryptAES256(returnData, System.Web.Configuration.WebConfigurationManager.AppSettings["Securekey"]+"RAJASTHAN_EGRASS.key",null);
                        message = UpdateStatus(plainText, GEN, amt.ToString());
                        break;
                    case "6390013":
                        ServicePointManager.Expect100Continue = true;
                       ...
                       // var response = string.Empty;
                        ...
                        cipherText = objEncry.EncryptAES256(plainText, System.Web.Configuration.WebConfigurationManager.AppSettings["Securekey"] + "ICICI.key");
                        InsertAudit(plainText, BankCode, ConfigurationManager.AppSettings["ServerName"].ToString() + " => " + cipherText, flag);
                        //BankUrl = ConfigurationManager.AppSettings["SBIWebServ.DoubleVerifyMerchantService"].Split('/').GetValue(2).ToString().Replace("merchant.", "");
                        ...
                        using (WebClient wc = new WebClient())
                        {
                            ...
                            returnData = wc.UploadString(URI, "POST", myParameters);
                        }


                        IPAddress = Dns.GetHostAddresses("apibankingone.icicibank.com")[0].ToString();

                        UploadData(BankCode, returnData, IPAddress, GRN, PaymentType);

                      
                        plainText = objEncry.DecryptAES256(returnData, System.Web.Configuration.WebConfigurationManager.AppSettings["Securekey"] + "ICICI.key", null);
                        message = UpdateStatus(plainText, GEN, amt.ToString());
